...
void appendSensorSnapshot(float* sensorVals, bool* includeFlags) {
  ...
  newEntry["RFID"] = rfid;  // Or use RTC time if available
  if (includeFlags[0]) newEntry["LUX"] = sensorVals[0];
  if (includeFlags[1]) newEntry["SOIL"] = sensorVals[1];
...
void loop(){
  ...
  if (!BLE_setup) {
    ...
    // Set BLE transmit power (can be from -21dBm to 9dBm)
    //esp_ble_tx_power_set(ESP_BLE_PWR_TYPE_DEFAULT, ESP_PWR_LVL_N9);

    //pServer->getAdvertising()->setMinPreferred(40); // Minimum interval (7.5ms)
    //pServer->getAdvertising()->setMaxPreferred(80); // Maximum interval (~30ms)
