...
async def ble_scan():
    ...
    async with aioble.scan(5000, interval_us=BLE_SCAN_INTERVAL, window_us=BLE_SCAN_WINDOW, active=True) as scanner:
    ...
    while True:
        ...
        async with connection:
            try:
                ...
                service = await asyncio.wait_for(connection.service(_SERVICE_UUID), timeout=5.0)
                # Discover the characteristic with a timeout
                characteristic = await asyncio.wait_for(service.characteristic(_CHARACTERISTIC_UUID), timeout=5.0)
